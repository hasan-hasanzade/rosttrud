 ...
 */            (function(window, factory) {
    ...
    (() => {
        ...
        function initSliders() {
            ...
            if (document.querySelector(".newsswiper")) new swiper_core_Swiper(".newsswiper", {
            ...
            if (document.querySelector(".eventswiper")) new swiper_core_Swiper(".eventswiper", {
                modules: [ Pagination, Autoplay ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
            ...
            if (document.querySelector(".usefullswiper")) new swiper_core_Swiper(".usefullswiper", {
                modules: [ Navigation, Autoplay ],
                observer: true,
                observeParents: true,
                slidesPerView: 5,
                ...
                effect: "fade",
                navigation: {
                    prevEl: ".swiper-button-prev-usefull",
                    nextEl: ".swiper-button-next-usefull"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 2.5,
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            if (items) {
                ...
                const buttons = document.querySelectorAll(".interface__btn");
                buttons.forEach((button => {
                    button.addEventListener("click", (function() {
                        console.log("message");
                        buttons.forEach((btn => btn.classList.remove("_nav-btn-active")));
                        this.classList.add("_nav-btn-active");
        ...
        document.addEventListener("mouseout", documentActions);
        const menuBlocks = document.querySelectorAll(".sub-menu-service__block");
        if (menuBlocks.length) menuBlocks.forEach((menuBlock => {
            const menuBlockItems = menuBlock.querySelectorAll(".sub-menu-service__category").length;
            menuBlock.classList.add(`sub-menu-service__block_${menuBlockItems}`);
        }));
        ...
        function documentActions(e) {
            ...
            if (targetElement.closest("[data-parent]")) {
                ...
                if (subMenu) {
                    if (currentParentElement && currentParentElement !== targetElement) {
                    ...
                    currentParentElement = targetElement;
                } else console.log("Ой ой, нет такого подменю :(");
                e.preventDefault();
            } else if (!targetElement.closest(".sub-menu-service__block")) {
                const activeLink = document.querySelector("._sub-menu-active");
                const activeBlock = document.querySelector("._sub-menu-open");
                if (activeLink) activeLink.classList.remove("_sub-menu-active");
                if (activeBlock) activeBlock.classList.remove("_sub-menu-open");
            ...
            if (targetElement.closest(".menu-top-header__link_service")) {
                document.documentElement.classList.add("service-open");
                e.preventDefault();
            }
            if (targetElement.closest(".menu-service__back")) {
            ...
            if (targetElement.closest(".sub-menu-service__back")) {
                document.documentElement.classList.remove("sub-menu-open");
                const activeLink = document.querySelector("._sub-menu-active");
                const activeBlock = document.querySelector("._sub-menu-open");
                if (activeLink) activeLink.classList.remove("_sub-menu-active");
        ...
        document.addEventListener("mouseover", headerServiceCards);
        document.addEventListener("mouseout", headerServiceCards);
        function headerServiceCards(e) {
            const targetElement = e.target;
            if (targetElement.closest("[data-parent-service]")) {
                const subMenuId = targetElement.dataset.parentService ? targetElement.dataset.parentService : null;
                const subMenu = document.querySelector(`[data-card="${subMenuId}"]`);
                if (subMenu) {
                    const activeLink = document.querySelector("._card-active");
                    const activeBlock = document.querySelector("._card-open");
                    if (activeLink && activeLink !== targetElement) {
                        activeLink.classList.remove("_card-active");
                        activeBlock.classList.remove("_card-open");
                        document.documentElement.classList.remove("card-open");
                    }
                    document.documentElement.classList.toggle("card-open");
                    targetElement.classList.toggle("_card-active");
                    subMenu.classList.toggle("_card-open");
                } else console.log("Ой ой, нет такого подменю :(");
                e.preventDefault();
            }
        }
        document.addEventListener("mouseover", headerMainActions);
        document.addEventListener("mouseout", headerMainActions);
        let currentMainParentElement = null;
        function headerMainActions(e) {
            const targetElement = e.target;
            if (targetElement.closest("[data-parent-main]")) {
                const subMenuId = targetElement.dataset.parentMain ? targetElement.dataset.parentMain : null;
                const subMenu = document.querySelector(`[data-main-submenu="${subMenuId}"]`);
                if (subMenu) {
                    if (currentMainParentElement && currentMainParentElement !== targetElement) {
                        const activeLink = document.querySelector("._sub-main-menu-active");
                        const activeBlock = document.querySelector("._sub-main-menu-open");
                        if (activeLink) activeLink.classList.remove("_sub-main-menu-active");
                        if (activeBlock) activeBlock.classList.remove("_sub-main-menu-open");
                        document.documentElement.classList.remove("sub-main-menu-open");
                    }
                    document.documentElement.classList.add("sub-main-menu-open");
                    targetElement.classList.add("_sub-main-menu-active");
                    subMenu.classList.add("_sub-main-menu-open");
                    currentMainParentElement = targetElement;
                } else console.log("Ой ой, нет такого подменю :(");
                e.preventDefault();
            } else if (!targetElement.closest(".sub-main-menu")) {
                const activeLink = document.querySelector("._sub-main-menu-active");
                const activeBlock = document.querySelector("._sub-main-menu-open");
                if (activeLink) activeLink.classList.remove("_sub-main-menu-active");
                if (activeBlock) activeBlock.classList.remove("_sub-main-menu-open");
                document.documentElement.classList.remove("sub-main-menu-open");
                currentMainParentElement = null;
            }
        }
        document.addEventListener("DOMContentLoaded", (function() {
            const buttons = document.querySelectorAll(".interface__btn");
            const articles = document.querySelectorAll(".interface__item");
            buttons.forEach((button => {
                button.addEventListener("click", (function() {
                    buttons.forEach((btn => btn.classList.remove("_nav-btn-active")));
                    button.classList.add("_nav-btn-active");
                    const filter = button.getAttribute("data-filter");
                    articles.forEach((article => {
                        const category = article.getAttribute("data-category");
                        if (filter === "all" || category === filter) {
                            article.classList.add("item-fade-enter");
                            setTimeout((() => {
                                article.classList.add("item-fade-enter-active");
                                article.classList.remove("item-fade-leave-active");
                            }), 10);
                            article.style.display = "block";
                        } else {
                            article.classList.remove("item-fade-enter-active");
                            article.classList.add("item-fade-leave-active");
                            setTimeout((() => {
                                article.classList.remove("item-fade-enter");
                            }), 300);
...
document.querySelectorAll('.radio input')[1].addEventListener('click', function() {
    if (this.checked) {
        document.querySelectorAll('.custom-radio')[1].classList.add('checked');
    } else {
        document.querySelectorAll('.custom-radio')[1].classList.remove('checked');
    }
});

document.querySelectorAll('.radio')[2].addEventListener('mouseover', function() {
    document.querySelectorAll('.custom-radio')[2].classList.add('checked');
});

document.querySelectorAll('.radio')[2].addEventListener('mouseout', function() {
    if (!document.querySelectorAll('.radio input')[2].checked) {
        document.querySelectorAll('.custom-radio')[2].classList.remove('checked');
    }
});
...
btn.addEventListener('click', () => {
    const navElement = document.querySelector('.interface');
    const regElement = document.querySelector('.double');
    const regionDesc = document.querySelector('.region__description');
    const doubleElements = document.querySelectorAll('.double__item');
    const doubleElements1 = document.querySelectorAll('.double__text');
    const regItems = document.querySelectorAll('.region__item');
    const regtexts = document.querySelectorAll('.region__item');
    const interface = document.querySelectorAll('.item-interface__body');

    if (!navElement) {
        console.error('Element with class "interface" not found for navElement.');
        return;
    }

    if (!regElement) {
        console.error('Element with class "interface" not found for regElement.');
        return;
    }

    if (doubleElements.length === 0) {
        console.error('Elements with class "double__item" not found.');
        return;
    }

    if (doubleElements1.length === 0) {
        console.error('Elements with class "double__text" not found.');
        return;
    }

   
    navElement.style.marginTop = '50px';
    regElement.style.marginTop = '50px';
    regionDesc.style.width = '100%';
    doubleElements.forEach(item => item.style.width = '100%');
    regItems.forEach(item => item.style.width = '100%');
    doubleElements1.forEach(item => item.style.maxWidth = '100%');
    regtexts.forEach(item => item.style.maxWidth = '100%');
    interface.forEach(item => item.style.height = '100%');

    const element = document.querySelector('.top-header__content');
    ...
    if (element) {
        ...
        console.error('Element with class "someClass" not found.');
    }
});